...
- 响应参数
  ...
  | 事件数据       | event_data | 包含tokenId和rodId（如果适用）的对象 | object   | {"tokenId": "1", "rodId": "2"} |
  | 钓手图片URL    | avatarPicUrl | 钓手NFT的图片URL（仅当type为avatar时） | string   | "https://example.com/avatar/1.png" |
  | 鱼竿图片URL    | rodPicUrl  | 鱼竿NFT的图片URL（仅当type为rod时） | string   | "https://example.com/rod/2.png" |
  ...
  - 成功响应
    ...
    ```json
    {
        ...
        "data": {
            ...
            "avatarPicUrl": "https://magenta-adorable-stork-81.mypinata.cloud/ipfs/QmaKvVRb8k1FQYbPZ38RfU2LJVCawwyd2Znf6ZSPkaDcJa/1.png"
        }
    }
    ```
...
4. 如果未铸造过：
   ...
   c. 获取交易哈希和事件数据（tokenId和rodId，如果适用）；
   d. 更新玩家的免费mint记录；
   e. 生成并返回相应的NFT图片URL；
5. 返回更新后的mint状态、交易哈希、事件数据和NFT图片URL（如果适用）。
...
6. 确保NFT图片URL的生成逻辑与实际存储位置一致；
7. 实现适当的错误处理，包括无效的用户ID、重复铸造等情况。
...
进入钓鱼准备界面，查询界面中相应的信息，信息基本都与玩家属性、玩家钓鱼资产状态相关。获取的内容包括：
...
- 响应参数
  ...
  | 返回数据 | data       | 玩家ID            | user_id          | 玩家唯一ID                                                   | string   | 123e4567-e89b-12d3-a456-426614174000 |
  ...
  | 返回数据 | data       | 玩家使用的钓手NFT | current_avatar_nft | 玩家当前使用的钓手nft信息                                | object   | {"tokenId": "NFT#23189", "avatarId": 1}      |
  | 返回数据 | data       | 玩家使用的鱼竿NFT | current_rod_nft  | 玩家当前使用的鱼竿NFT信息                                | object   | {"tokenId": "NFT#23189", "rodId": 1}         |
  | 返回数据 | data       | 拥有的所有钓手NFT | owned_avatar_nfts | 玩家拥有的所有钓手NFT信息列表，未拥有则为空数组     | array    | [{"tokenId": "NFT#23189", "avatarId": 1}, {"tokenId": "NFT#23190", "avatarId": 2}] |
  | 返回数据 | data       | 拥有的所有鱼竿NFT | owned_rod_nfts   | 玩家拥有的所有鱼竿NFT信息列表，未拥有则为空数组    | array    | [{"tokenId": "NFT#23189", "rodId": 1}, {"tokenId": "NFT#23190", "rodId": 2}] |
  | 返回数据 | data       | 鱼竿Battle Skill  | battle_skill_desc_en | 鱼竿Battle Skill英文描述                                | string   | An ordinary fisherman, diligently practicing. |
  | 返回数据 | data       | 鱼竿QTE Skill     | qte_skill_desc_en | 鱼竿QTE Skill英文描述                                   | string   | Plain Fishing Rod                            |
  ...
  | 返回数据 | data       | 当前钓鱼次数      | fishing_count    | 玩家当前的钓鱼次数                                           | integer  | 5                                            |
  | 返回数据 | data       | 下次恢复时间      | next_recovery_time | 下次钓鱼次数恢复的时间戳（秒）                               | integer  | 1623456789                                   |
